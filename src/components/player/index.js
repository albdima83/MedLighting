...
export const setUpVideoTexture = () => {
  if (!ApplicationInstance.tag('VideoTexture')) {
    const el = ApplicationInstance.c({
      type: VideoTexture(),
      ref: 'VideoTexture',
      zIndex: 0,
      videoEl,
    })
    this.childList.addAt(el, 0)
  }
...
const autoSetupMixin = (sourceObject, setup = () => {}) => {
  let ready = false
...
export default autoSetupMixin(videoPlayerPlugin, () => {
  precision = 1 //(AppInstance && AppInstance.stage && AppInstance.stage.getRenderPrecision()) || precision
  videoEl = setupVideoTag()
  textureMode = true //Settings.get('platform', 'textureMode', false)
  if (textureMode === true) {
